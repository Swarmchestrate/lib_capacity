...
if __name__ == "__main__":
    ...
    """
    #Marking a set of resources as "allocated" for demonstration purposes, should succeed
    swarmid="swarm2"
    msid="ratings_v1"
    offerid=list(offers[msid].keys())[0]
    res_set = capreg.resource_set_get_from_offer(offerid, offers[msid][offerid])
    if res_set is not None:
        capreg.resource_set_deployed(swarmid, msid, res_set["restype"], res_set["resid"], res_set["count"])
        capreg.dump_capacity_registry_info() 

    #Marking a set of resources as "allocated" for demonstration purposes, should fail
    swarmid="swarm3"
    msid="ratings_v1"
    offerid=list(offers[msid].keys())[0]
    res_set = capreg.resource_set_get_from_offer(offerid, offers[msid][offerid])
    if res_set is not None:
        ...
        print("Failed to get resource set from offer, cannot deploy resources")


    #Marking a set of resources back to "assigned" for demonstration purposes, should succeed
    swarmid="swarm2"
    msid="ratings_v1"
    offerid=list(offers[msid].keys())[0]
    res_set = capreg.resource_set_get_from_offer(offerid, offers[msid][offerid])
